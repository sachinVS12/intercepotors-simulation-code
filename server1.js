...
client.on("connect", () => {
  console.log("MQTT Publisher started (SWEEP MODE 1Hz → 300Hz)");
...
function publishMessage() {
  try {
    ...
    console.log(
      `Published OK | Header=(${frameA},${frameB}) | SweepFreq=${signalFrequency}Hz | Tacho=${tachoFixedFreq}Hz`,
    );

    // -------- UPDATE SWEEP --------
    signalFrequency++;
    if (signalFrequency > maxFrequency) {
      signalFrequency = 1; // restart sweep
    }
  } catch (err) {
